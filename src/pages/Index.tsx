...
/** Observe the <html> class list so inline styles can react to dark mode. */
...
const Index = () => {
  ...
  const sectionWhite: React.CSSProperties = isDark
    ? {}
    : {};

  const liveDemoBg: React.CSSProperties = isDark
    ? {}
    : { background: "linear-gradient(180deg, #f5f3ff 0%, #ffffff 100%)" };

  const stepCardBg = (i: number): React.CSSProperties =>
    isDark
      ? { background: i === 1 ? "rgba(109,40,217,0.15)" : "transparent" }
      : { background: i === 1 ? "linear-gradient(145deg, #faf5ff, #ede9fe)" : "white" };

  const featureCardBg = (f: typeof features[0]): React.CSSProperties =>
    ({ backgroundColor: isDark ? f.darkBg : f.bg });

  const ctaBadge: React.CSSProperties = isDark
    ? { background: "rgba(109,40,217,0.25)", color: "#C4B5FD" }
    : { background: "#EDE9FE", color: "#7C3AED" };

  const ctaOutlineBtn: React.CSSProperties = isDark
    ? { borderColor: "#7C3AED", color: "#C4B5FD" }
    : { borderColor: "#C4B5FD", color: "#7C3AED" };

  const iconBox: React.CSSProperties = isDark
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      ...
      {/* ── HOW IT WORKS ─────────────────────────────────────────── */}
      <section className="py-16 md:py-24 bg-white dark:bg-slate-950" style={sectionWhite}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            ...
            {steps.map((step, i) => (
              <motion.div key={step.n} {...fadeUp(0.1 * i)}
                className="relative rounded-2xl p-7 border border-violet-100 dark:border-violet-800 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all"
                style={stepCardBg(i)}>
      ...
      {/* ── LIVE DEMO ─────────────────────────────────────────────── */}
      <section className="py-16 md:py-24 bg-violet-50 dark:bg-slate-900" style={liveDemoBg}>
        <div className="container px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <motion.div key={f.title} {...fadeUp(0.07 * i)}
                className={`rounded-2xl p-7 hover:shadow-lg hover:-translate-y-1 transition-all border ${isDark ? "border-slate-700" : "border-gray-100"} ${f.wide ? "sm:col-span-2 lg:col-span-2" : ""}`}
                style={featureCardBg(f)}>
                <div className="w-12 h-12 rounded-xl shadow-sm flex items-center justify-center mb-5" style={iconBox}>
                  {f.icon}
                </div>
